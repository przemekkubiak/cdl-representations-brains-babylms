...
#   pipeline/data/processed/fmri/<ds>/roi-motor/<Task>/session_rdm_<ses>.npz
# but run_roi_sweeps.sh looks for them under
#   <root>/data/ds003604-session-rdms/<ds>/roi-motor/<Task>/session_rdm_<ses>.npz
...
waves_running(){ pgrep -f run_motor_chained.sh >/dev/null 2>&1; }
...
run_sweep(){   # $1 = label
  ...
  GPU="${GPU:-1}" DATASETS="$DS" bash "$ROOT/scripts/run_roi_sweeps.sh" roi-motor
...
# Re-push the registration cache: it now also holds the roi-motor masks and
# QC overlays. The affines themselves are unchanged (ROI-independent), so this
# only adds the new ROI-specific artefacts.
say "pushing updated registration cache (now includes roi-motor masks)"
