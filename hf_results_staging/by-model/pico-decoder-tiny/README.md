...
**pico decoder, smallest of the scale ladder** — [`pico-lm/pico-decoder-tiny`](https://huggingface.co/pico-lm/pico-decoder-tiny) · dense commit-history checkpoints.

21 checkpoints, steps 0–126000.
...
|---|---|---|
| brain RSA (mean over checkpoints × 12 cells) | -0.0167 | ❌ confounded |
| brain RSA vs training step | rho -0.016, p 0.799 (n=252) | ❌ confounded |
| interp: participation ratio (effective dim / hidden) | 0.2209 | ✅ |
| interp: gini (activation sparsity) | 0.2042 | ✅ |
| interp: hoyer sparsity | 0.0933 | ✅ |
| interp: CKA to previous checkpoint | 0.9353 | ✅ |
| localisation: selectivity index | 0.5459 | ✅ |
| localisation: mean overlap with other phenomena | 0.0030 | ✅ |
| localisation: active layers | 7.0595 | ✅ |
| localisation: layer centre of mass | 0.6129 | ✅ |
| behaviour: minimal-pair accuracy (chance 0.5) | 0.6311 | ✅ |

## What this model shows

- Minimal-pair accuracy **0.631**, modestly above chance.
- Participation ratio **0.221**: the representation uses ~22% of its available dimensions. Mid-range for this release.
- Selectivity **0.546** — like every model in this release, phenomena are only moderately isolated. Nothing here shows sharp, dedicated linguistic circuitry.
- CKA to the previous checkpoint reaches 0.9911 by the end: **the representation has stopped changing**, so late checkpoints are near-duplicates.
...
![overview](figures/pico-decoder-tiny_overview.png)
