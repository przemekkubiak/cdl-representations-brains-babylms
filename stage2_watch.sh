...
#   pipeline/data/processed/fmri/<ds>/roi-phonology/<Task>/session_rdm_<ses>.npz
# but run_roi_sweeps.sh looks for them under
#   <root>/data/ds003604-session-rdms/<ds>/roi-phonology/<Task>/session_rdm_<ses>.npz
...
waves_running(){ pgrep -f run_masked_waves.sh >/dev/null 2>&1; }
...
run_sweep(){   # $1 = label
  ...
  GPU="${GPU:-1}" DATASETS="$DS" bash "$ROOT/scripts/run_roi_sweeps.sh" roi-phonology
