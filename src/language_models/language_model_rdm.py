...
class LanguageModelRDMComputer:
    ...
    def compute_stimulus_rdm(
        ...
        stimulus_embeddings = []
        fallback_count = 0
        # Zero-vector fallback keeps stimulus count aligned with brain RDM size.
        zero_vec = np.zeros(embeddings.shape[1], dtype=np.float32)
        
        for idx, row in characteristics.iterrows():
            word_a = row.get("word_A")
            word_b = row.get("word_B")
            
            # Normalize token strings; for missing entries fall back to zero vectors.
            word_a = str(word_a).strip().lower() if not pd.isna(word_a) else ""
            word_b = str(word_b).strip().lower() if not pd.isna(word_b) else ""

            emb_a = embeddings[word_to_idx[word_a]] if word_a in word_to_idx else zero_vec
            emb_b = embeddings[word_to_idx[word_b]] if word_b in word_to_idx else zero_vec
        ...
        if len(stimulus_embeddings) == 0:
            raise ValueError("No stimuli available for RDM computation")
        
        stimulus_embeddings = np.array(stimulus_embeddings)
        logger.info(
            f"Computed {len(stimulus_embeddings)} stimulus embeddings "
